...
function ansReview($qntypeid){
	...
	for ($k=0; $k < $count; $k++) { 
		...
		$query1 = "SELECT * FROM ".$tmp_tblname." where questiontypeid = " . $qntypeid . " order by id OFFSET ".$k. " LIMIT 1";
		...
		$questionids_for_qntype[$k] = $record[0];
		unset($row);
		unset($record);
	}
		...
		for($i = 0; $i < count($questionids_for_qntype); $i++){
			$j = $i+1;
			echo "<span style= 'font-face:sans-serif;font-size: 15px; font-weight: normal;'>".$j.".</span>";

			$filenameReview = getFilenameReview($questionids_for_qntype[$i]);

			//echo "<pre>";
			/*print_r($filenameReview[0]);
			print_r($filenames_for_qntype[$i]);*/
			//print_r($questionids_for_qntype[$i]);
			//echo "</pre>";
			...
			if($filenameReview[0][$imgtype_flag] == 0){
				echo "<span style= 'font-face:sans-serif;font-size: 15px; font-weight: bold;'>" . $filenameReview[
				0][$concat_flag] . "</span><br /><br />";
				fetchChoicesWORadio($questionids_for_qntype[$i]);
				echo "<br /><br /><br />";
			}
			else{
				$imgfile = $filenameReview[0][$concat_flag];
				$maxversion = getContentversion($imgfile);
				$content = dispContent($imgfile, $maxversion);
				fetchChoicesWORadio($questionids_for_qntype[$i]);
			}
			unset($filenames_for_qntype);
		}
...
function fetchChoicesWORadio($questid){
	...
	for ($i=0; $i < $choice_count; $i++) { 
		...
		if($j==$CC){
			...
			if($CC != $RC){
				echo "<img src = 'images/tickBlue.png' width='30px' height='30px' />";
			}
		}else{
	...
	for ($i=0; $i < $choice_count; $i++) { 
		...
		if($j == $CC || $j == $RC){
			...
			if($j == $RC){
				...
				if($choicetype[$i] == 0){
					echo "<center><span style='font-family:sans-serif;color:green;'>Right Choice: &nbsp;&nbsp;&nbsp;". $choicetxt[$i] . "</span></center><br />";
				}
				else{
				$choice_filename = $choicefile[$i];
				//url for the file..
